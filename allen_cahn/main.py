...
def training_data_latin_hypercube(
    X, T, U_gt, N_boundary=200, N_inner=1e3, lb=None, ub=None
):
    # boundary conditions
...
class sequential_model(nn.Module):
    ...
    def function(self, x):
        ...
        u_xxtt = autograd.grad(
            u_xt, g, torch.ones(x.shape).to(self.device), create_graph=True
        )[0]
